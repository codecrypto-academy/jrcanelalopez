...
docker run --rm -v ${PWD}/data:/data -v ${PWD}/genesis.json:/genesis.json ethereum/client-go:v1.13.15 init --datadir /data /genesis.json


Cambiar las address de unlock y miner.etherbase

docker run -p 8545:8545 -d --name eth-node1 -v ${PWD}/data:/data -v ${PWD}/password.txt:/password.txt ethereum/client-go:v1.13.15 --ipcdisable --nodiscover --datadir /data --networkid 3333 --allow-insecure-unlock --unlock 7e50fa8509af04b463f519ea673f580a5b93ff76 --password /password.txt --mine --miner.etherbase 7e50fa8509af04b463f519ea673f580a5b93ff76 --http --http.api personal,admin,eth,net,web3,debug,txpool,miner --http.addr 0.0.0.0 --http.port 8545 --http.corsdomain="*" --http.vhosts="*"

```

# Ver lineas del dockers

```
docker logs eth-node1 | tail -30

```

# Parar nodo y borrarlo
...
```
curl -s -X POST http://localhost:8545 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"eth_getBalance","params":["0x318E4E8a30e2ba06E27097c37e8212905835E140","latest"],"id":1}' | python3 -m json.tool
```

# Ver transacciones pendientes

## Ver el status para saber el ultimo

```
curl -s -X POST http://localhost:8545 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"txpool_status","params":[],"id":1}' | python3 -m json.tool
```

```
curl -s -X POST http://localhost:8545 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"txpool_content","params":[],"id":1}' | python3 -m json.tool
```

```
curl -s -X POST http://localhost:8545 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' | python3 -c "import sys,json; print(f\"Bloque actual: {int(json.load(sys.stdin)['result'], 16)}\")"
...
# Verificar que existe el contracto

```
curl -s -X POST http://localhost:8545 \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"eth_getCode","params":["0x00F0895504D2fC10901aAddb7f0367Fad9110aFC","latest"],"id":1}' | python3 -m json.tool
```
