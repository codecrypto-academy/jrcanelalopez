...
const kafka = new Kafka({
  clientId: "horneado-service",
  brokers: [KAFKA_BROKER],
});

const consumer = kafka.consumer({ groupId: "horneado-group" });
...
async function initBlockchain() {
  ...
  // Verificar que la cuenta tiene el rol HORNEADOR
  const accountRole = await contract.roles(wallet.address);
  if (Number(accountRole) !== Role.HORNEADOR) {
    throw new Error(
      `❌ Account ${wallet.address} does not have HORNEADOR role (has role ${accountRole})`
    );
  }

  const balance = await provider.getBalance(wallet.address);
  console.log(`✅ Connected as HORNEADOR`);
...
async function processMessage(message) {
  try {
    ...
    // Verificar que es para HORNEADO
    if (payload.targetType !== "HORNEADO") {
      console.log(
        `⚠️  Skipping - target is ${payload.targetType}, not HORNEADO`
      );
      return;
    }

    // Crear token HORNEADO con los dos tokens MOLIENDA como padres
    console.log(
      `\n🍞 Creating HORNEADO token with parents [${payload.tokenId1}, ${payload.tokenId2}]...`
    );
    ...
    if (tokenCreatedEvent) {
      ...
      console.log(`\n🎉 HORNEADO token created successfully!`);
      ...
      // Publicar evento para el siguiente servicio (EMBALAJE)
      await publishHorneadoEvent(payload.tokenId1, newTokenId);
    } else {
...
// Publicar evento HORNEADO para el siguiente servicio
async function publishHorneadoEvent(tokenId1, tokenId2) {
  try {
    const payload = {
    ...
    await producer.send({
      topic: "horneado-event",
      messages: [{ value: JSON.stringify(payload) }],
    });

    console.log(`\n📤 Published to Kafka:`);
    console.log(`   Topic: horneado-event`);
...
// Main
async function main() {
  console.log("🚀 Starting HORNEADO Service...\n");
  ...
  try {
    ...
    await consumer.subscribe({
      topic: process.env.KAFKA_TOPIC || "molienda-event",
      fromBeginning: false, // Solo nuevos mensajes
    });
    console.log(
      `👂 Subscribed to topic: ${process.env.KAFKA_TOPIC || "molienda-event"}\n`
    );
