...
const kafka = new Kafka({
  clientId: "embalaje-service",
  brokers: [KAFKA_BROKER],
});

const consumer = kafka.consumer({ groupId: "embalaje-group" });
...
async function initBlockchain() {
  ...
  // Verificar que la cuenta tiene el rol EMBALADOR
  const accountRole = await contract.roles(wallet.address);
  if (Number(accountRole) !== Role.EMBALADOR) {
    throw new Error(
      `❌ Account ${wallet.address} does not have EMBALADOR role (has role ${accountRole})`
    );
  }

  const balance = await provider.getBalance(wallet.address);
  console.log(`✅ Connected as EMBALADOR`);
...
async function processMessage(message) {
  try {
    ...
    // Verificar que es para EMBALAJE
    if (payload.targetType !== "EMBALAJE") {
      console.log(
        `⚠️  Skipping - target is ${payload.targetType}, not EMBALAJE`
      );
      return;
    }

    // Crear token EMBALAJE con los dos tokens HORNEADO como padres
    console.log(
      `\n📦 Creating EMBALAJE token with parents [${payload.tokenId1}, ${payload.tokenId2}]...`
    );
    ...
    if (tokenCreatedEvent) {
      ...
      console.log(`\n🎉 EMBALAJE token created successfully!`);
      ...
      // Publicar evento para el siguiente servicio (DISTRIBUCION)
      await publishEmbalajeEvent(payload.tokenId1, newTokenId);
    } else {
...
// Publicar evento EMBALAJE para el siguiente servicio
async function publishEmbalajeEvent(tokenId1, tokenId2) {
  try {
    const payload = {
    ...
    await producer.send({
      topic: "embalaje-event",
      messages: [{ value: JSON.stringify(payload) }],
    });

    console.log(`\n📤 Published to Kafka:`);
    console.log(`   Topic: embalaje-event`);
...
// Main
async function main() {
  console.log("🚀 Starting EMBALAJE Service...\n");
  ...
  try {
    ...
    await consumer.subscribe({
      topic: process.env.KAFKA_TOPIC || "horneado-event",
      fromBeginning: false, // Solo nuevos mensajes
    });
    console.log(
      `👂 Subscribed to topic: ${process.env.KAFKA_TOPIC || "horneado-event"}\n`
    );
