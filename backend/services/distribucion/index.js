...
const kafka = new Kafka({
  clientId: "distribucion-service",
  brokers: [KAFKA_BROKER],
});

const consumer = kafka.consumer({ groupId: "distribucion-group" });
...
async function initBlockchain() {
  ...
  // Verificar que la cuenta tiene el rol DISTRIBUIDOR
  const accountRole = await contract.roles(wallet.address);
  if (Number(accountRole) !== Role.DISTRIBUIDOR) {
    throw new Error(
      `❌ Account ${wallet.address} does not have DISTRIBUIDOR role (has role ${accountRole})`
    );
  }

  const balance = await provider.getBalance(wallet.address);
  console.log(`✅ Connected as DISTRIBUIDOR`);
...
async function processMessage(message) {
  try {
    ...
    // Verificar que es para DISTRIBUCION
    if (payload.targetType !== "DISTRIBUCION") {
      console.log(
        `⚠️  Skipping - target is ${payload.targetType}, not DISTRIBUCION`
      );
      return;
    }

    // Crear token DISTRIBUCION con los dos tokens EMBALAJE como padres
    console.log(
      `\n� Creating DISTRIBUCION token with parents [${payload.tokenId1}, ${payload.tokenId2}]...`
    );
    ...
    if (tokenCreatedEvent) {
      ...
      console.log(`\n🎉 DISTRIBUCION token created successfully!`);
      ...
      // Publicar evento para el siguiente servicio (VENTA)
      await publishDistribucionEvent(payload.tokenId1, newTokenId);
    } else {
...
// Publicar evento MOLIENDA para el siguiente servicio
// Publicar evento DISTRIBUCION para el siguiente servicio
async function publishDistribucionEvent(tokenId1, tokenId2) {
  try {
    const payload = {
      tokenId1: Number(tokenId1),
    ...
    await producer.send({
      topic: "distribucion-event",
      messages: [{ value: JSON.stringify(payload) }],
    });

    console.log(`\n📤 Published to Kafka:`);
    console.log(`   Topic: distribucion-event`);
...
// Main
async function main() {
  console.log("🚀 Starting DISTRIBUCION Service...\n");
  ...
  try {
    ...
    await consumer.subscribe({
      topic: process.env.KAFKA_TOPIC || "embalaje-event",
      fromBeginning: false, // Solo nuevos mensajes
    });
    console.log(
      `👂 Subscribed to topic: ${process.env.KAFKA_TOPIC || "embalaje-event"}\n`
    );
