...
const kafka = new Kafka({
  clientId: "almacen-service",
  brokers: [KAFKA_BROKER],
});

const consumer = kafka.consumer({ groupId: "almacen-group" });
...
async function initBlockchain() {
  ...
  // Verificar que la cuenta tiene el rol ALMACENADOR
  const accountRole = await contract.roles(wallet.address);
  if (Number(accountRole) !== Role.ALMACENADOR) {
    throw new Error(
      `❌ Account ${wallet.address} does not have ALMACENADOR role (has role ${accountRole})`
    );
  }

  const balance = await provider.getBalance(wallet.address);
  console.log(`✅ Connected as ALMACENADOR`);
...
async function processMessage(message) {
  try {
    ...
    // Verificar que es para ALMACEN
    if (payload.targetType !== "ALMACEN") {
      console.log(
        `⚠️  Skipping - target is ${payload.targetType}, not ALMACEN`
      );
      return;
    }

    // Crear token ALMACEN con los dos tokens COSECHA como padres
    console.log(
      `\n🏭 Creating ALMACEN token with parents [${payload.tokenId1}, ${payload.tokenId2}]...`
    );
    ...
    if (tokenCreatedEvent) {
      ...
      console.log(`\n🎉 ALMACEN token created successfully!`);
      ...
      // Publicar evento para el siguiente servicio (MOLIENDA)
      await publishAlmacenEvent(newTokenId);
    } else {
...
// Publicar evento ALMACEN para el siguiente servicio
async function publishAlmacenEvent(tokenId) {
  try {
    const payload = {
    ...
    console.log(`\n📤 Published to Kafka:`);
    console.log(`   Topic: almacen-event`);
...
// Main
async function main() {
  console.log("🚀 Starting ALMACEN Service...\n");
  ...
  try {
    ...
    console.log("👂 Subscribed to topic: cosecha-event\n");
