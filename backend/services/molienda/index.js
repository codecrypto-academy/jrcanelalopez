...
const kafka = new Kafka({
  clientId: "molienda-service",
  brokers: [KAFKA_BROKER],
});

const consumer = kafka.consumer({ groupId: "molienda-group" });
...
async function initBlockchain() {
  ...
  // Verificar que la cuenta tiene el rol MOLINERO
  const accountRole = await contract.roles(wallet.address);
  if (Number(accountRole) !== Role.MOLINERO) {
    throw new Error(
      `❌ Account ${wallet.address} does not have MOLINERO role (has role ${accountRole})`
    );
  }

  const balance = await provider.getBalance(wallet.address);
  console.log(`✅ Connected as MOLINERO`);
...
async function processMessage(message) {
  try {
    ...
    // Verificar que es para MOLIENDA
    if (payload.targetType !== "MOLIENDA") {
      console.log(
        `⚠️  Skipping - target is ${payload.targetType}, not MOLIENDA`
      );
      return;
    }

    // Crear token MOLIENDA con los dos tokens ALMACEN como padres
    console.log(
      `\n� Creating MOLIENDA token with parents [${payload.tokenId1}, ${payload.tokenId2}]...`
    );
    ...
    if (tokenCreatedEvent) {
      ...
      console.log(`\n🎉 MOLIENDA token created successfully!`);
      ...
      // Publicar evento para el siguiente servicio (HORNEADO)
      await publishMoliendaEvent(payload.tokenId1, newTokenId);
    } else {
...
async function publishMoliendaEvent(tokenId1, tokenId2) {
  try {
    const payload = {
    ...
    await producer.send({
      topic: "molienda-event",
      messages: [{ value: JSON.stringify(payload) }],
    });

    console.log(`\n📤 Published to Kafka:`);
    console.log(`   Topic: molienda-event`);
...
// Main
async function main() {
  console.log("🚀 Starting MOLIENDA Service...\n");
  ...
  try {
    ...
    await consumer.subscribe({
      topic: process.env.KAFKA_TOPIC || "almacen-event",
      fromBeginning: false, // Solo nuevos mensajes
    });
    console.log(
      `👂 Subscribed to topic: ${process.env.KAFKA_TOPIC || "almacen-event"}\n`
    );
